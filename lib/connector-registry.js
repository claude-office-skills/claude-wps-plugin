...
const CREDENTIALS_PATH = join(homedir(), ".claude-wps", "connector-credentials.json");
...
class ConnectorRegistry {
  ...
  async pull(connectorId, action, params) {
    ...
    const credentials = allCreds[connectorId] || {};
    ...
    for (const [id, conn] of this._connectors) {
      result.push({
        ...
        hasCredentials: (() => {
          ...
          return required.every((c) => !!creds[c.key]);
        })(),
      });
    }
    return result;
  ...
  setCredentials(connectorId, creds) {
    const allCreds = loadCredentials();
    allCreds[connectorId] = { ...(allCreds[connectorId] || {}), ...creds };

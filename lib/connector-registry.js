...
class ConnectorRegistry {
  ...
  async pull(connectorId, action, params) {
    ...
    try {
      const result = await conn.handler.pull({
        ...
        credentials,
        cache: cacheStore,
        config: conn.manifest._config || null,
      });
  ...
  async createUserConnector(definition) {
    const { id, name, description, baseUrl, authType, authHeaderName, authQueryParam, authCredentialKey, credentials: credDefs, endpoints } = definition;
    if (!id || !name || !baseUrl) {
      return { ok: false, error: "缺少必填字段: id, name, baseUrl" };
    }
    if (this._connectors.has(id)) {
      return { ok: false, error: `连接器 "${id}" 已存在` };
    }

    const connDir = join(USER_DIR, id);
    ensureDir(connDir);
    ...
    const manifest = {
      ...
      name,
      version: "1.0.0",
      description: description || `用户自定义连接器: ${name}`,
      credentials: (credDefs || []).map((c) => ({
        key: c.key || "api_key",
        label: c.label || c.key || "API Key",
        required: c.required !== false,
      })),
      actions: Object.entries(endpoints || {}).map(([actionId, ep]) => ({
        id: actionId,
        name: ep.name || actionId,
      ...
      _type: "generic-rest",
      _config: { baseUrl, authType, authHeaderName, authQueryParam, authCredentialKey, endpoints },
    };

    writeFileSync(join(connDir, "manifest.json"), JSON.stringify(manifest, null, 2), "utf-8");

    const genericHandlerSrc = readFileSync(resolve(BUNDLED_DIR, "_generic-rest", "handler.js"), "utf-8");
    writeFileSync(join(connDir, "handler.js"), genericHandlerSrc, "utf-8");

    await this.reload();
    return { ok: true, id, message: `连接器 "${name}" 已创建` };
  }

  async removeUserConnector(connectorId) {
    const conn = this._connectors.get(connectorId);
    if (!conn) return { ok: false, error: `连接器 "${connectorId}" 未找到` };
    if (conn.layer !== "user") return { ok: false, error: "仅可删除用户自定义连接器" };

    const { rmSync: rm } = await import("fs");
    try { rm(conn.dir, { recursive: true }); } catch {}
    vault.remove(connectorId);
    this._connectors.delete(connectorId);
    return { ok: true, message: `连接器 "${connectorId}" 已删除` };
  }

...
function classifyActivity(a: ActivityEvent): ToolCardType {
  ...
  if (a.action === "execute_code" || a.action === "code_execution")
    return "expanded";
  if (a.action === "tool_use" && (a.name === "Bash" || a.name === "Write" || a.name === "Edit"))
    return "expanded";
  return "inline";
}
...
function getPhrase(action: string, name: string): string {
  ...
  if (action === "tool_use") return `使用 ${name}`;
...
function InlineCard({
  ...
  return (
    <div className={styles.inlineCard}>
      ...
      {isStreaming && isLast ? (
        <span className={blockStyles.statusRunning}>正在{phrase}...</span>
      ) : (
        <span>{phrase}</span>
      )}
    </div>
...
function ToolCallRenderer({ activities, isStreaming }: Props) {
  ...
  const title = isStreaming
    ? `使用 ${activities.length} 个工具...`
    : `已使用 ${activities.length} 个工具`;
        ...
        {classified
          ...
          .map((c, i) => (
            <InlineCard
              ...
              activity={c.activity}
              isLast={
                i === classified.filter((x) => x.type === "inline").length - 1
              }
              isStreaming={isStreaming}
            />
          ))}
